...
export function runPoupaiV25({ extraction, marketResult, availabilityChecks = [], engineConfig = {}, metrics = {} } = {}) {
  ...
  if (!readerAudit.safeToUse && !validationErrors.length) {
    blockingReasons.push(blocker(
      ...
      'A leitura da fatura não atingiu a confiança mínima para uma decisão segura.',
      'reader',
    ));
  }
  ...
  if (blockingReasons.length) {
    ...
    return {
      ...baseMeta(),
      status: blockingReasons.some((x) => ['reader', 'billing_baseline'].includes(x.stage)) ? 'REVIEW_BILL' : 'ANALYSIS_INCONCLUSIVE',
      readerValidation,
      readerAudit,
      billFreshness,
      billingBaseline,
      ...
      analysis: null,
      finalDecision,
      decisionReason: blockingReasons.length > 1 ? 'MULTIPLE_BLOCKERS' : blockingReasons[0].code,
  ...
  if (!marketGate.canRunPreliminaryEngine) {
    ...
    return {
      ...
      billFreshness,
      billingBaseline,
      blockingReasons: [blocker('INSUFFICIENT_MARKET_DATA', marketGate.message, 'market')],
      marketGate,
  ...
  const analysis = analyzeStructuredInternetBill({
    ...
    offers: engineOffers,
    config: { ...engineConfig, asOfDate: engineConfig.asOfDate || hardenedMarket?.checkedAt || null },
  });
  ...
  return {
    ...
    decisionConfidence: finalDecision === 'ANALISE_INCONCLUSIVA' ? 'preliminary' : 'final',
